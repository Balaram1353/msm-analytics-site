...
**Client logo chip** (`.client-logo-chip`, `--lg` and `--crop` modifiers) — pairs with `.card--work__client` inside a shared `.card--work__header` row, on the homepage cards, both detail-page-header instances (`--lg`, 88px) of each page's own client, and every "More case studies" teaser card. The chip exists to solve a real inconsistency, not for decoration: the three source logo files (`assets/logos/`) were prepared three different ways — MSIG's is a genuinely transparent WebP, QuantivRisk's JPEG has a baked near-white background, Zywave's JPEG has a baked dark-navy background — and displayed edge-to-edge they'd read as three different *kinds* of image sitting next to each other, not one component with three fills. The chip's own white circular background + border normalizes all three into one frame without editing any source file. Tokens: `--client-logo-chip-size` (40px, cards) / `--client-logo-chip-size-lg` (88px, headers) — don't fit the icon scale (tops out at 56px) or its named steps, so they're their own two-value scale (mirrored in the design-tokens contract block above, per that block's own sync rule).

`--crop` is MSIG-only and has a real declaration-order dependency: it sets `padding: 0` to override `--lg`'s `padding: var(--space-2)` on the large header variant, so `--crop` must be declared *after* `--lg` in the stylesheet (it is) — reordering them would silently break MSIG's header-scale crop back to a letterboxed/padded lockup. The modifier exists because MSIG's source asset is a portrait mark+wordmark lockup, not a square icon like the other two; `object-fit: cover` crops it live to just the diamond mark (source file untouched) so it matches the other two's optical weight, and incidentally avoids MSIG being spelled out twice in the same cluster (the lockup's own wordmark, right next to the `.card--work__client` tag that already says "MSIG").
